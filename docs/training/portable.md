...
Portable mode is a self-contained PyInstaller build. It runs without the Lab source checkout and stores settings, chat/history database, logs and the API-key file in `Sentinel Fork Data` on the removable drive. It never silently falls back to the Mac's Application Support folder.
...
Open **Start Sentinel Fork.command**. The launcher checks that the drive still exists, is writable and has enough free space. Directly opening the app also recognizes the adjacent marker. Clear errors replace silent fallback when the volume is read-only, missing or nearly full.

To update, run the same build command against the same destination. It replaces the generated app but preserves `Sentinel Fork Data`. Back up that entire folder before an important upgrade.
...
**Completion check:** locate `Sentinel Fork Data/.env` and `data/sentinel.db` on the removable volume, confirm no new Sentinel data appeared under Application Support, quit the app, and eject safely.
