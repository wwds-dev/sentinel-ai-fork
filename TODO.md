...
- [ ] `P1` `feature` `@ai` **Finish integrating VPN Agent** through its canonical `agents/vpn_agent/` companion repo. Tunnel now imports the shared chat/config implementation and has read-only diagnostics, selected-profile comparison, and non-executing action previews. Real local key generation, private config parsing, separately gated privileged execution, backup/restore entry points, and full standalone-app parity remain. Keep the companion as the implementation owner rather than copying its services into Sentinel.
...
- [ ] `P2` `feature` `security` `@ai` **v3 staged specialist integrations.** Tunnel's profile-aware read-only diagnostics and safe previews are done. Bloodhound metadata/rule matching comes first; deeper private-key-free Tunnel config parsing and gated actions follow; Trace public-source adapters next; then authorised Bug Spray assessment and passive Beacon analysis. Exclude denial-of-service, credential theft, stealth/persistence and uncontrolled exploitation.
